...
def test_add_other_data():
    new_announcement1 = Announcement(title="Ogłoszenie1", description="Opis1", date=date(2001, 1, 1), readerVisibility=True)
    new_announcement2 = Announcement(title="Ogłoszenie2", description="Opis2", date=date(2001, 1, 2), readerVisibility=True)
    new_announcement3 = Announcement(title="Ogłoszenie3", description="Opis3", date=date(2001, 1, 3), readerVisibility=True)
    db.session.add(new_announcement1)
    db.session.add(new_announcement2)
    db.session.add(new_announcement3)
    new_event1 = Event(name="Wydarzenie1", description="Opis1", date=date(2002, 2, 1))
    new_event2 = Event(name="Wydarzenie2", description="Opis2", date=date(2002, 2, 2))
    new_event3 = Event(name="Wydarzenie3", description="Opis3", date=date(2002, 2, 3))
